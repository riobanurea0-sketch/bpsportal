...
st.set_page_config(page_title="Sensus Ekonomi | BPS Dairi", page_icon="🏢", layout="wide")
...
<style>
    ...
    .module-tag { font-size: 0.85rem; color: #3b82f6; font-weight: 500; letter-spacing: 0.1em; text-transform: uppercase; margin-bottom: 0.5rem; font-family: monospace;}
...
<div class="module-header">
    <div class="module-tag">MODUL ANALITIK &middot; 01</div>
    <div class="module-title">Sensus Ekonomi.</div>
    <div class="module-desc">Pusat analisis data kegiatan usaha dan makroekonomi wilayah. Tumpuk beberapa dataset untuk analisis tren komprehensif.</div>
</div>
""", unsafe_allow_html=True)

# 3. Inisialisasi Memori (Sektor Ekonomi)
if 'history_ekonomi' not in st.session_state:
    st.session_state['history_ekonomi'] = {}
...
with col_upload:
    ...
    file_sensus = st.file_uploader("Unggah berkas Ekonomi (CSV/XLSX)", type=["csv", "xlsx", "xls"], accept_multiple_files=True, key="upload_ekonomi")
    
    if file_sensus:
        with st.spinner('Menyuntikkan matriks data...'):
            for f in file_sensus:
                if f.name not in st.session_state['history_ekonomi']:
                    try:
                        if f.name.endswith('.csv'):
                            df_temp = pd.read_csv(f)
                        else:
                            df_temp = pd.read_excel(f)
                        st.session_state['history_ekonomi'][f.name] = df_temp
    ...
    if len(st.session_state['history_ekonomi']) == 0:
        st.info("Belum ada berkas di memori.")
    else:
        for nama_file in list(st.session_state['history_ekonomi'].keys()):
            col_nama, col_hapus = st.columns([4, 1])
            col_nama.write(f"📄 `{nama_file}`")
            if col_hapus.button("❌", key=f"del_eko_{nama_file}"):
                del st.session_state['history_ekonomi'][nama_file]
                st.rerun()

with col_analisis:
    if len(st.session_state['history_ekonomi']) > 0:
        df = pd.concat(st.session_state['history_ekonomi'].values(), ignore_index=True)
        st.session_state['data_ekonomi'] = df # Simpan untuk dibaca panel Global
        
        col_m1, col_m2, col_m3 = st.columns(3)
        col_m1.metric("Total Baris Gabungan", f"{df.shape[0]:,}")
        col_m2.metric("Total File Tersimpan", f"{len(st.session_state['history_ekonomi'])}")
        ...
        with tab1:
            ...
            with col_cfg:
                if len(kolom_kategori) > 0 and len(kolom_angka) > 0:
                    sumbu_x = st.selectbox("X Axis (Kategori)", kolom_kategori, key="x_ekonomi")
                    sumbu_y = st.selectbox("Y Axis (Angka)", kolom_angka, key="y_ekonomi")
                    jenis_grafik = st.radio("Tipe Render", ["Bar Chart", "Line Chart", "Area Chart"], key="chart_ekonomi")
                else:
        ...
        st.session_state['data_ekonomi'] = None
