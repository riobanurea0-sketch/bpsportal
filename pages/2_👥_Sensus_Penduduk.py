...
st.set_page_config(page_title="Sensus Penduduk | BPS Dairi", page_icon="👥", layout="wide")
...
<style>
    ...
    .module-tag { font-size: 0.85rem; color: #f59e0b; font-weight: 500; letter-spacing: 0.1em; text-transform: uppercase; margin-bottom: 0.5rem; font-family: monospace;}
...
<div class="module-header">
    <div class="module-tag">MODUL ANALITIK &middot; 02</div>
    <div class="module-title">Sensus Penduduk & Demografi.</div>
    <div class="module-desc">Pusat analisis data populasi, ketenagakerjaan, dan kesejahteraan sosial. Tumpuk beberapa dataset untuk analisis tren komprehensif.</div>
</div>
""", unsafe_allow_html=True)

# 3. Inisialisasi Memori (Sektor Penduduk)
if 'history_penduduk' not in st.session_state:
    st.session_state['history_penduduk'] = {}
...
with col_upload:
    ...
    file_sensus = st.file_uploader("Unggah berkas Penduduk (CSV/XLSX)", type=["csv", "xlsx", "xls"], accept_multiple_files=True, key="upload_penduduk")
    
    if file_sensus:
        with st.spinner('Menyuntikkan matriks data...'):
            for f in file_sensus:
                if f.name not in st.session_state['history_penduduk']:
                    try:
                        if f.name.endswith('.csv'):
                            df_temp = pd.read_csv(f)
                        else:
                            df_temp = pd.read_excel(f)
                        st.session_state['history_penduduk'][f.name] = df_temp
    ...
    if len(st.session_state['history_penduduk']) == 0:
        st.info("Belum ada berkas di memori.")
    else:
        for nama_file in list(st.session_state['history_penduduk'].keys()):
            col_nama, col_hapus = st.columns([4, 1])
            col_nama.write(f"📄 `{nama_file}`")
            if col_hapus.button("❌", key=f"del_pen_{nama_file}"):
                del st.session_state['history_penduduk'][nama_file]
                st.rerun()

with col_analisis:
    if len(st.session_state['history_penduduk']) > 0:
        df = pd.concat(st.session_state['history_penduduk'].values(), ignore_index=True)
        st.session_state['data_penduduk'] = df # Simpan untuk dibaca panel Global
        
        col_m1, col_m2, col_m3 = st.columns(3)
        col_m1.metric("Total Baris Gabungan", f"{df.shape[0]:,}")
        col_m2.metric("Total File Tersimpan", f"{len(st.session_state['history_penduduk'])}")
        ...
        with tab1:
            ...
            with col_cfg:
                if len(kolom_kategori) > 0 and len(kolom_angka) > 0:
                    sumbu_x = st.selectbox("X Axis (Kategori)", kolom_kategori, key="x_penduduk")
                    sumbu_y = st.selectbox("Y Axis (Angka)", kolom_angka, key="y_penduduk")
                    jenis_grafik = st.radio("Tipe Render", ["Bar Chart", "Line Chart", "Area Chart"], key="chart_penduduk")
                else:
        ...
        st.session_state['data_penduduk'] = None
